function rew= evalu_throwing_473_OL(inp,ai,para)
...
velocita=diff(output_NN_pos')/0.01;
% calcolare parabola oggetto a partire dal punto di lancio
% trovare (kt, at^2+bt+c, t)
g=9.81;
t=0:0.01:0.50; % 10 sec per lanciare
basket=para.basket;
rew=inf;
release=0;
for i=1:length(velocita)

    y=velocita(i,2)*t-1/2*g*t.^2+output_NN_pos(2,i);
    z=velocita(i,3)*t+output_NN_pos(3,i);
    parabola_t=[y;z];
    
    [xy,distance,t_a] = distance2curve(parabola_t',basket');%,interpmethod);
