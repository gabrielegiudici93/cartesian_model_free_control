...
from setup_fun import setup_LSTM,setup_NARX, setup_gazebo_LSTM,setup_gazebo_NARX, setup_LSTM_open,setup_LSTM_close
...
wrist_offset= +12.6
step=50
exp=10000 #numero esperimento
task='THROWING' #REACHING THROWING
...
if gazebo=='off':
    if setup=='LSTM':
        ...
        setup_LSTM_close(iCtrl,iPos, iEnc)
        enco_initial_pos()
...
#il file reference deve essere assolutamente (2 o 3) x step
    ...
    for i in range(iterations):
        ...
        if i==1:
            setup_LSTM_open(iCtrl,iPos,iEnc)

        tempo_effettivo = time.time() - start_time

        if tempo_effettivo>0.0:
    ...
    df_position.to_csv('OPTIMIZATION/icub_result/'+setup+'_icub_validation' + str(exp) + '_THROWING.csv', sep=' ',decimal=',', index=False)
